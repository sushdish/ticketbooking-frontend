...
import {getUserRewards, pigination } from "../../admin/helper/adminapicall";
...
const RewardPoints = () => {
    ...
    const handlePagination = async (event , newPage) => {
        ...
        await getUserRewards(newPage).then((data) => {
          if (data.err) {
            console.log(data.err);
          } else {
            setRewardPoints(data.rewardPoint);
        ...
        <TableHead>
            ...
            {rewardPoints.map((points) => (
                     <TableRow key={points._id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                        ...
                        <TableCell component="th" scope="row">
                            <div style={{ color: points.RewardPoints >= 0 ? 'green' : 'red' }}>
                               {points.RewardPoints}
                            </div>
